...
create table tblMemo (
    ...
    memo varchar2(1000) not null,   --메모내용(N·N)
...
insert into tblMemo (seq, name, memo, regdate) 
    values (2, '고양이', '', sysdate); -- 빈문자열도 null로 취급한다.
...
insert into tblMemo (seq, name, memo, regdate) 
    values (3, '홍길동', '메모 테스트', sysdate);   

insert into tblMemo (seq, name, memo, regdate) 
    values (null, '홍길동', '메모 테스트', sysdate);  
...
--명시적 null 대입 > 사용자의 의지가 담겨있다. + 의지 표현
insert into tblMemo (seq, name, memo, regdate) 
    values (2, null, '멍멍멍.', sysdate); --null > default 동작 안함

--암시적 null 대입 > 사용자의 의지가 없다. + 권리 포기
insert into tblMemo (seq, memo, regdate) 
    values (3, '멍멍멍.', sysdate); --null 대신 '익명' > default 동작 함
...
/*
    ...
    1. 컬럼 수준에서 만드는 방법
        - 여태까지의 수업 방식
        - 컬럼을 선언할 때 제약 사항도 같이 선언하는 방식
        - 가독성 향상 > 어떤 컬럼 + 어떤 제약 > 직관적으로 인식 가능
    
    2. 테이블 수준에서 만드는 방법
        - 컬럼 선언과 제약 사항 선언을 분리하는 방식
        - 가독성 향상 > 테이블 크기가 커질 수록 코드 관리 용이
